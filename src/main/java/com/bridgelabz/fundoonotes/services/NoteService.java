...
public class NoteService implements INoteService {
	...
	public ResponseDTO updateNote(NoteDTO noteDTO, String token, int id) {
		...
		ResponseDTO response = new ResponseDTO("Success", note);
		return response;
	}
	...
	public ResponseDTO deleteNote(String token, int id) {
		...
		if (note.isTrash() == true) {
			...
			response = new ResponseDTO("Deletetion Success", note);
		} else {
	...
	public ResponseDTO pinNote(String token, int id) {
		...
		if (note.isPin() == false) {
			note.setPin(true);
			noteRepository.save(note);
			response = new ResponseDTO("Pinned", note);
	...
	public ResponseDTO trashNote(String token, int id) {
		...
		if (note.isTrash() == false) {
			note.setTrash(true);
			noteRepository.save(note);
			response = new ResponseDTO("Trash", note);
		} else if (note.isTrash() == true) {
			note.setTrash(false);
			noteRepository.save(note);
			response = new ResponseDTO("Retrieve from trash", note);
		} else {
	...
	public ResponseDTO archiveNote(String token, int id) {
		...
		if (note.isArchive() == false) {
			...
			noteRepository.save(note);
			response = new ResponseDTO("Archived Note", note);
		} else if (note.isArchive() == true) {
			note.setArchive(false);
			noteRepository.save(note);
			response = new ResponseDTO("unarchive Note", note);
		} else {
